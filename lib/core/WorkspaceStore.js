...
import { PLUGIN_STORAGE_KEY, PLUGIN_DEFAULT_GAME_VERSION } from '@/lib/config/plugins';
import {
  WORKSPACE_DEFAULTS,
...
function reviveServer(entry) {
  ...
  return {
    ...
    plugins: {
      ...
      gameVersion: entry.plugins?.gameVersion ?? PLUGIN_DEFAULT_GAME_VERSION,
  ...
  if (pluginsRaw?.servers && typeof pluginsRaw.servers === 'object') {
    ...
    entries.forEach(([name, server], index) => {
      ...
      servers[id].plugins = {
        ...
        gameVersion: server.gameVersion ?? PLUGIN_DEFAULT_GAME_VERSION,
...
export class WorkspaceStore {
    ...
    Object.entries(this.servers).forEach(([id, server]) => {
      servers[id] = {
        ...
        plugins: {
          ...
          gameVersion: server.plugins.gameVersion,
  ...
  getPluginGameVersion(serverId = this.activeServerId) {
    return this.servers[serverId]?.plugins.gameVersion ?? PLUGIN_DEFAULT_GAME_VERSION;
  }

  setPluginGameVersion(gameVersion, serverId = this.activeServerId) {
    const server = this.servers[serverId];
    if (!server) return;
    server.plugins.gameVersion = gameVersion;
  }
