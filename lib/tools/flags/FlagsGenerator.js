import { FLAG_DEFAULTS, FLAG_IGNORE_JAVA_VERSION, FLAG_RESTART, FLAG_UTF8_ENCODING } from '@/lib/config/flags.js';
...
export class FlagsGenerator {
  ...
  javaCommand(state, options = {}) {
    ...
    if (state.ignoreJavaVersion && FlagsSoftware.supportsIgnoreJavaVersion(state.software)) {
      parts.push(FLAG_IGNORE_JAVA_VERSION);
    }
