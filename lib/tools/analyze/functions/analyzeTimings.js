...
import { buildOutdatedVersionField } from '@/lib/tools/analyze/checks/versionCheck';

import config_bukkit from '@/lib/tools/analyze/configs/bukkit';
import plugins_paper from '@/lib/tools/analyze/configs/plugins/paper';
import plugins_purpur from '@/lib/tools/analyze/configs/plugins/purpur';
import config_purpur from '@/lib/tools/analyze/configs/purpur';
import config_server_properties from '@/lib/tools/analyze/configs/serverProperties';
import servers from '@/lib/tools/analyze/configs/servers';
import config_spigot from '@/lib/tools/analyze/configs/spigot';
import config_paper_27 from '@/lib/tools/analyze/configs/timings/paper-27';
import config_paper_28 from '@/lib/tools/analyze/configs/timings/paper-28';
import config_pufferfish from '@/lib/tools/analyze/configs/timings/pufferfish';

export default async function analyzeTimings(id) {
  ...
  try {
    ...
    return [{ name: '❌ Processing Error', value: `MTools cannot process this spark profile. Please use an alternative spark profile. ${err}` }];
  }
  ...
  if (timing_cost > 300) {
    fields.push({ name: '❌ Timingcost', value: `Your timingcost is ${timing_cost}. Your cpu is overloaded and/or slow. Look for better hosting.` });
  }

  const outdatedField = await buildOutdatedVersionField(version);
  if (outdatedField) fields.push(outdatedField);

  if (TIMINGS_CHECK.servers.servers) {
    TIMINGS_CHECK.servers.servers.forEach((server) => {
  ...
  if (cpu <= 2) fields.push({ name: '❌ Threads', value: `You only have ${cpu} thread(s). Look for better hosting.` });
  ...
  if (timing_cost > 500) {
    ...
    return [{ name: '❌ Timingcost (URGENT)', value: `Your timingcost is ${timing_cost}. This value would be at most 200 on a reasonable server. Your cpu is critically overloaded and/or slow. Hiding ${suggestions} comparitively negligible suggestions until you resolve this fundamental problem. Look for better hosting.` }];
  }
