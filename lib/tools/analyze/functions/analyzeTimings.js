...
export default async function analyzeTimings(id) {
  ...
  if (timing_cost > 300) {
    fields.push({ name: '❌ Timingcost', value: `Your timingcost is ${timing_cost}. Your cpu is overloaded and/or slow.`, buttons: [{ text: 'Find a better host', url: 'https://modrinth.black' }] });
  }
  ...
  if (cpu <= 2) fields.push({ name: '❌ Threads', value: `You only have ${cpu} thread(s).`, buttons: [{ text: 'Find a better host', url: 'https://modrinth.black' }] });
