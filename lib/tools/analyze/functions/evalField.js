...
import { extractConfigFileFromValue } from '@/lib/tools/analyze/configYamlPath';
...
export default function evalField(fields, option, option_name, plugins, server_properties, bukkit, spigot, paper, pufferfish, purpur) {
  ...
  option.forEach((option_data) => {
    ...
    option_data.configKey = option_name;
    option_data.configFile = extractConfigFileFromValue(option_data.value);
