...
import { analyzeSparkMetrics } from '@/lib/tools/analyze/checks/sparkMetrics';
import { analyzeSparkProfiler } from '@/lib/tools/analyze/checks/sparkProfiler';
...
import config_paper from '@/lib/tools/analyze/configs/profile/paper';
...
export default async function analyzeProfile(id) {
    ...
    return [
      {
        ...
        value: `MTools is unable to process this spark profile. It appears that the platform is not supported for analysis. Platform: ${platform}`,
      },
    ];
  }
  const outdatedField = await buildOutdatedVersionField(mcversion);
  if (outdatedField) fields.push(outdatedField);
  const brand = sampler.metadata.platform.brand;
  if (PROFILE_CHECK.servers.servers) {
    PROFILE_CHECK.servers.servers.forEach((server) => {
  ...
  fields.push(...analyzeSparkMetrics(sampler));
  fields.push(...await analyzeSparkProfiler(id, sampler.metadata));
