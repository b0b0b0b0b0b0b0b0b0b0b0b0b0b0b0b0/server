...
const SEVERITY_RANK = { issue: 0, warn: 1, ok: 2 };
...
export function prioritizeAnalyzeResults(results) {
  return [...results].sort((left, right) => {
    const severityDelta =
      SEVERITY_RANK[classifyFindingSeverity(left)] - SEVERITY_RANK[classifyFindingSeverity(right)];
    if (severityDelta !== 0) return severityDelta;
    return getFindingPriority(left) - getFindingPriority(right);
  });
}
