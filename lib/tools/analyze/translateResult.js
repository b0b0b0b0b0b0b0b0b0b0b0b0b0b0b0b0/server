...
const VALUE_PATTERNS = [
  ...
  {
    re: /^You only have (\d+) thread\(s\)\.$/,
    key: 'threadCount',
    params: (match) => ({ count: match[1] }),
  },
  {
    re: /^Your timingcost is (\d+)\. Your cpu is overloaded and\/or slow\.$/,
    key: 'timingcost',
    params: (match) => ({ cost: match[1] }),
  },
  {
    re: /^Your timingcost is (\d+)\. This value would be at most 200 on a reasonable server\. Your cpu is critically overloaded and\/or slow\. Hiding (\d+) comparitively negligible suggestions until you resolve this fundamental problem\.$/,
  ...
  {
    re: /^CPU is (.+) with (\d+) thread\(s\)\. Minecraft wants fast single-thread performance, not a cheap VPS\.$/,
    key: 'virtualCpu',
...
export function translateAnalyzeResults(results, t) {
  return results.map((field) => ({
    ...
    buttons: field.buttons?.map((button) => ({
      ...button,
