...
const VALUE_PATTERNS = [
  ...
  },
  {
    ...
    params: (match, t) => ({
      entity: translatePathSegment(match[1], t),
      file: match[2],
      value: match[3],
    }),
  },
...
function translateConfigPathName(name, t) {
  ...
  const translated = match[2].split('.').map((segment) => translatePathSegment(segment, t));
...
function translateValue(value, t) {
  for (const pattern of VALUE_PATTERNS) {
    ...
    if (match) {
      return translateKey(t, 'tools.analyze.results.patterns', pattern.key, pattern.params(match, t));
    }
  }
