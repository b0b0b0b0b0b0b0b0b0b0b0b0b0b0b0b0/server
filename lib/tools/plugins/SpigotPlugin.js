function parseSpigotTimestamp(value) {
...
function mapSpigotVersion(version) {
  return {
    id: version.id,
    name: version.name,
    releaseDate: parseSpigotTimestamp(version.releaseDate),
  };
}
...
export class SpigotPlugin {
  ...
  fromData(data) {
    ...
    this.releaseDate = parseSpigotTimestamp(data.releaseDate);
    this.updateDate = parseSpigotTimestamp(data.updateDate);
  ...
  async fetch(fetchOptions) {
    const { gameVersion } = normalizePluginFetchOptions(fetchOptions);
    await this.fetchData();
    await this.fetchVersions();
    this.targetGameVersion = gameVersion;
    this.noBuildForGameVersion = false;
    return this;
  }
