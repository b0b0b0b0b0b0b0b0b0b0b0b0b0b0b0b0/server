...
async function searchCatalog(type, query, loaders) {
  await yieldToMain();
  if (type === 'modrinth') {
    return searchPlugins('modrinth', query, loaders);
  }
  return searchPlugins('spigot', query);
}

async function resolveModrinth(searchNames, version, software) {
  ...
  for (const query of searchNames) {
    ...
    const hits = await searchCatalog('modrinth', query, loaders);
    hits.forEach((hit) => {
...
async function resolveSpigot(searchNames, version) {
  const hitsById = new Map();
  let bestSoFar = null;
...
async function resolveParsedJar(item, software) {
  const { name, version, fileName } = item;
  const searchNames = jarSearchNames({ name, fileName });

  try {
    const modrinth = await resolveModrinth(searchNames, version, software);
    if (modrinth) {
      return { ...item, plugin: modrinth, source: 'modrinth' };
    }

    await yieldToMain();
    const spigot = await resolveSpigot(searchNames, version);
    if (spigot) {
...
export async function resolveJarPlugins(parsedItems, existingPlugins, software, { onProgress, onItem } = {}) {
  const seen = new Set();
  const results = [];
  ...
  for (let index = 0; index < parsedItems.length; index += 1) {
    ...
    const resolved = await resolveParsedJar(parsedItems[index], software);
